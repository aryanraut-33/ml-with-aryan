...
import styles from '../blogs/blogs.module.css'; // Using the unified stylesheet
        ...
        {videos.map((video) => (
          <Link href={`/videos/${video._id}`} key={video._id} className={styles.card}>
            <div className={styles.cardImage}>
              {video.videoUrl && <Image src={getYoutubeThumbnail(video.videoUrl)} alt={video.title} fill style={{ objectFit: 'cover' }} />}
              {user && (
                <div className={styles.interactionOverlay}>
                  <LikeButton contentId={video._id} contentType="video" isInitiallyLiked={interactions.liked.includes(video._id)} />
                  <BookmarkButton contentId={video._id} contentType="video" isInitiallyBookmarked={interactions.bookmarked.includes(video._id)} />
                </div>
              )}
            </div>
            <div className={styles.cardContent}>
              <h2 className={styles.cardTitle}>{video.title}</h2>
              <p className={styles.cardMeta}>{new Date(video.createdAt).toLocaleDateString()}</p>
              <div className={styles.cardFooter}>
                <span className={styles.views}>{video.views || 0} views</span>
